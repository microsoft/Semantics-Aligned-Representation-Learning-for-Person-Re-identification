...
Person re-identification (reID) aims to match person images to retrieve the ones with the same identity. It has applications such as people tracking for finding lost child, visitor density analysis in retail store. Person reID is a challenging task, as the images to be matched are generally semantically misaligned due to the diversity of human poses and capture viewpoints, incompleteness of the visible bodies (due to occlusion), etc. 
